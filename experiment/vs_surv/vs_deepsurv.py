...
def resnet18(args):
    model = ResNet1d(BasicBlock1d, layers=[2, 2, 2, 2], input_channels=args.input_channels, num_classes=1)
    return model
...
def setup_parser():
    ...
    parser.add_argument('--model_name', default='vs_deepsurv.net',
                        help='model name')
...
class VsDatasetBatch(Dataset):
    def __init__(self, data_path, time, event):
        ...
        self.time, self.event = tt.tuplefy(np.array(time), np.array(event)).to_tensor()
...
def get_vs_data(datasetBatch):
    vs_array = []
    data_path_list = datasetBatch.data_path
    for p in data_path_list:
        vs = pd.read_csv(p)
        vs = vs.drop(vs.columns[[0, 1, 2]], axis=1)
        data = np.array(vs).astype('float32')
        data = torch.from_numpy(data)
        vs_array.append(data)
    return torch.stack(vs_array)
...
def main():
    ...
    net = resnet18(args)
    model = CoxPH(net, tt.optim.Adam(lr=args.lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=5e-4, amsgrad=False))
    ...
    _ = model.compute_baseline_hazards(get_vs_data(dataset_train), (dataset_train.time, dataset_train.event))
    ...
    ev = EvalSurv(surv, np.array(labels_test[0]), np.array(labels_test[1]), 'km')
